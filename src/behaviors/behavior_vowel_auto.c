 ...
 * CONFIG_ZMK_AML_TYPING_MODE_TIMEOUT, tap the consonant via direct HID
 * reports BEFORE raising the vowel keycode event.
 */
...
static int on_press(struct zmk_behavior_binding *binding,
    ...
    if (g_typing_timer != 0 &&
        (event.timestamp - g_typing_timer) <= CONFIG_ZMK_AML_TYPING_MODE_TIMEOUT &&
